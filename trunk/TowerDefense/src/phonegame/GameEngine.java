...
public abstract class GameEngine extends MIDlet implements Runnable
{
    ...
    private final void detectItemCollisions()
    {
        ...
        {  	
            ...
            {
                window.checkForItemCollisions(it);
            }
        }
    }
     ...
     * <pre>
     * <li>
     * <pre>PLAYER_FIXED</pre>
     * <br/>
     * Viewport won't move. If your world is bigger than the screen, the player can
     * move out of sight on all sides!
     * <li>
     * <pre>PLAYER_TOP, PLAYER_VCENTER, PLAYER_BOTTOM</pre>
     * <br/>
     * Player is at top, center or bottom of viewport. Horizontal positioning is not
     * controled, you can move out of view at the sides!
     * <li>
     * <pre>PLAYER_LEFT, PLAYER_HCENTER, PLAYER_RIGHT</pre>
     * <br/>
     * Player is at left, center or right of viewport. Vertical positioning is not
     * controled, you can move out of view at the top or bottom!
     * <li>
     * <pre> PLAYER_TOP | PLAYER_LEFT, PLAYER_TOP | PLAYER_HCENTER, PLAYER_TOP | PLAYER_RIGHT
     * <br/>
     * PLAYER_VCENTER | PLAYER_LEFT, PLAYER_VCENTER | PLAYER_HCENTER, PLAYER_VCENTER | PLAYER_RIGHT
     * <br/>
     * PLAYER_BOTTOM | PLAYER_LEFT, PLAYER_BOTTOM | PLAYER_HCENTER, PLAYER_BOTTOM | PLAYER_RIGHT</pre>
     * <br/>
     * Combinations of vertical and horizontal positioning. 
     * The following image illustrates this type of positioning: 
     * <br/>
     * <img src="playerpos.jpg">
     * </ul>
     */
    ...
    * @param tileWidth
     ...
     * <br/>
     * The tilenumber is a byte value. It must be smaller than 256. Also you need
     * to use a type cast, when you enter an integer number.
     * 
     ...
     * and height. 
     * <p/>
     * Note: to find the tile at a certain point, fill out 1 for width and height. An zero-width area 
    ...
    private class GameWindow extends Canvas implements CommandListener
    {
        ...
        private void drawTileEnvironment(Graphics g)
        {
            if (map != null && tileImages != null)
            {
                ...
                for (int heightMap = 0; heightMap < mapHeight; heightMap++)
                {
                    for (int widthMap = 0; widthMap < mapWidth; widthMap++)
                    {
                        tileType = map[heightMap][widthMap];
                        // If in viewport
                        //if (xPos + tileWidth > viewPortX && xPos <
                        // screenWidth + viewPortX &&
                        // yPos + tileHeight > viewPortY && yPos < screenWidth +
                        // viewPortY) {
         ...
         */
        private final void checkForItemCollisions(MoveableGameItem currentItem)
        {
            // loop through all other (static and moveable) gameitems that
            // are visible in the viewport
            for (int i = 0; i < gameItems.size(); i++)
            {
                GameItem anotherItem = (GameItem) gameItems.elementAt(i);
                if (currentItem != anotherItem && anotherItem.isActive() )
                {
        ...
        { 	// 
            ...
            // System.out.println("col2, left: " + moveleft + ", up:" + moveup + "  " + gameItem.dirString());
